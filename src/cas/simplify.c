                            ...
                            phy_ir_ref *out_ref)
{
    ...
    if (numeric_base && exact_base.num == 0) {
        ...
        if (phy_cas_exact_value(cas, exponent, &exact_exponent)) {
                                 ...
                                 phy_ir_ref argument, phy_ir_ref *out_ref)
{
    ...
    if (head == cas->fn_sin || head == cas->fn_cos || head == cas->fn_tan) {
        if (zero_argument) {
            *out_ref = (head == cas->fn_cos) ? cas->one : cas->zero;
            return PHY_OK;
        }
        /*
        ...
        if (phy_cas_known_function(cas, argument) == cas->fn_log) {
