...
static int transportEnv(const char *name, int fallback, int minimum, int maximum)
{
...
static int ackTimeoutMs()
{
	return transportEnv("NSPIRE_CX2_ACK_TIMEOUT_MS", 2000, 50, 60000);
}

static int handshakeTimeoutMs()
{
	return transportEnv("NSPIRE_CX2_HANDSHAKE_TIMEOUT_MS", 5000, 50, 60000);
}

static int ackAttempts()
{
	return transportEnv("NSPIRE_CX2_ACK_RETRIES", 4, 1, 20);
}
...
int packet_send_cx2(struct nspire_handle *nsp_handle, char *data, int size)
{
	...
	int ret = -NSPIRE_ERR_BUSY;
	const int maxlen = sizeof(NNSEMessage) + 1472;
	NNSEMessage * const message = reinterpret_cast<NNSEMessage*>(malloc(maxlen));
	bool acked = false;

	/*
	 * Upstream sent a packet once and then waited for as many as ten
	 * 60-second reads. If the calculator received the packet but usbipd
	 * dropped its ACK, no further packet could arrive and a transfer looked
	 * hung for minutes. NNSE defines reqAck bit 0x08 as "not the first
	 * try"; resend the same sequence number with that bit set and use a
	 * bounded wait for each attempt.
	 */
	for(int attempt = 0; attempt < ackAttempts() && !acked; ++attempt)
	{
		msg->reqAck = uint8_t(attempt == 0 ? 0x01 : 0x09);
		if(!writePacket(handle, msg))
			continue;

		/*
		 * Usually the first read is the ACK. Permit a few address/time
		 * service packets without multiplying a missing-ACK delay by ten.
		 */
