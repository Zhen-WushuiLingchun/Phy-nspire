...
uint32_t packet_max_datasize(nspire_handle_t *h) {
	/*
	 * The native CX II transport can carry 1440-byte CSP payloads, but a
	 * smaller packet is useful when the USB device is forwarded through
	 * usbipd/WSL. Keep upstream's value as the default and let the host-side
	 * deployment tool select a conservative value before opening the device.
	 */
	if (h->is_cx2) {
		const char *value = getenv("NSPIRE_CX2_PACKET_SIZE");
		char *end = NULL;
		...
		if (end && *end == '\0' && parsed >= 64 && parsed <= 1440)
