...
- host smoke test — done; the suite covers the platform, relative pointer,
  ...
  full lifecycle: Windows 34/34, WSL ASan/UBSan/leak 36/36, and 213,505
  explicit checks;
- generated `.tns` size report — 1,154,912 bytes, 18.4% of the 6 MiB ceiling,
...
- CAS unit tests — done, `tests/test_cas.c`, 2,531 checks covering the normal
...
- notebook tests — done, `tests/test_notebook.c`, 215 checks covering bounded
  ...
  fixture;
- evaluator tests — done, `tests/test_eval.c`, 1,776 checks. The physics cases
...
- formula bridge tests — done, `tests/test_formula.c`, 46 checks covering
  initialization, matrices, metrics, RGB565 rendering, and local error
  recovery;
- source, palette and pointer tests — done: 398 source-language checks
  including assignment and reserved-head canonicalization, 863 palette checks
...
The real Ndless r2022/ARM GNU toolchain link check is done for the CAS: 33/33
CAS APIs survive garbage collection and the probe packages to a 121,640-byte
...
product is 1,154,912 bytes. The independent SU(N) colour probe retains 23/23
