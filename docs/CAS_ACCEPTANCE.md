...
13,240-byte `PHYNB001` notebook with 204 source cards:

- seventeen Markdown cells with nMarkdown LaTeX;
- 187 editable Math inputs;
...
IR trees during `FILE > Open`; running all inputs produces 187 typed outputs and
a 391-card session. The inputs touch every currently implemented evaluator
...
| exact linear algebra | dynamic `Vector` / `Matrix`, `Dot`, `Determinant`, `CharacteristicPolynomial`, `Eigenvalues`, `Eigenvectors`, `Eigenspace`, `GeneralizedEigenspace`, `JordanDecomposition`, `Inverse`, `RowReduce`, `MatrixRank`, `LinearSolve`, `Transpose`, `Dimensions` |
...
- Current WSL GCC Release and combined ASan/UBSan/leak suites: 48/48 each.
  Focused ball/CAS/evaluator tests contain 39,364, 33,840, and 3,259 checks
...
- Canonical complex-algebraic Wolfram oracle: 13/13 for root order,
  conjugation, minimal polynomials, resultant/reader-`Root` closure,
  characteristic polynomials, eigenvectors, generalized spaces, Jordan
  conventions, and eigenvalue multiplicity.
- Assertion-bearing executables: 478,792 checks.
- Notebook MENU completeness: every supported evaluator/source command is
  present in ten scrollable CAS categories.
- Ndless r2022 ARM product: 1,291,027 bytes, 20.5% of the 6 MiB ceiling.
...
- Isolated real/complex-algebraic ARM probe: 54/54 public APIs, 49,080 bytes
  of algebraic text and a 95,012-byte package.
- Isolated CAS ARM probe: 41/41 public APIs, 143,772 bytes of CAS text,
  276,300-byte package, and no float formatter, libm call, or ARM soft-float
  helper.
- Isolated evaluator ARM probe: 17/17 public APIs, 55,994 bytes of evaluator
  text, 462,364-byte package, and no float formatter, libm call, or ARM
  soft-float helper.
